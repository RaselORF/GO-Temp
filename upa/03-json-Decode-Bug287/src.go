...
func (b *builder) typeSwitchStmt(s *ast.TypeSwitchStmt, label *lblock) {
	...
	for _, clause := range s.Body.List {
		...
		for _, casetype := range cc.List {
			...
			//	 }
			//
			// Without adding type node into block, cfg will look like this :
			//
			//	            msg string
			//	               |
			//	            n.(type)
			...
			//	              return res
			//
			// After adding type node in block, the cfg will look like
			//
			//	            msg string
			//	               |
			//	            n.(type)
...
// There will be multiple edge according to CommClause count.
// Edge edge refers to a select body with respective CommClause expression
// as first node.
//
...
func (b *builder) selectStmt(s *ast.SelectStmt, label *lblock) {
	...
	currentBody := b.current
	for _, cc := range s.Body.List {
		clause := cc.(*ast.CommClause)
		// If default body is empty, we wont create a block
		if clause.Comm == nil && clause.Body == nil {
			continue
		}
		newBody := b.newBlock("select.body")
		// Add newBody as successor of current block
		b.jump(newBody)
		// Jump to newBody
		b.current = newBody
		b.targets = &targets{
		...
		b.targets = b.targets.tail
		// Add done block as successor of newBody
		b.jump(done)
		b.current = currentBody
	}
	b.current = done
}
