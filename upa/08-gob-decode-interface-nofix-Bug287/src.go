...
type CFG struct {
	...
	fset        *token.FileSet
	// Pending map
	pendingMap           map[ast.Node]*set.Set[ast.Node]
	pendingResolutionMap map[ast.Node][]SuccessorNode
}
...
// addSuccessor adds the successor for the node. Also adds the node as
// the predecessor of the successor node.
func (fg *CFG) addToPendingMap(node, pending ast.Node) {
	if existing, ok := fg.pendingMap[node]; ok {
		existing.Add(pending)
	} else {
		existing = set.OfElemets(pending)
		fg.pendingMap[node] = existing
	}
}
...
// 		if res, ok := fg.processedBlks[curr]; ok {
...
// 			flNode := FlowNode{Node: curr.Nodes[0]}
// 			entries = append(entries, flNode)
// 			fg.processedBlks[curr] = flNode
// 			continue
// 		}

//			// If we reach here, we haven't found any entry.
//			// So, process the successor blocks
//			for i := len(curr.Succs) - 1; i >= 0; i-- {
//				workList.AddFirst(curr.Succs[i])
//			}
//		}
//		return entries
//	}
...
func (fg *CFG) FixCfgForSelect2(sel *ast.SelectStmt) {
	comms := sel.Body.List
	n := len(comms)
	if n == 0 {
		// There is no CommClause inside.
	...
	predOfSelect := fg.predecessorMap[FlowNode{Node: firstCom.Comm}]
	fmt.Println(predOfSelect)
	multiSuccessor := fg.mayHaveMultipleSuccessor(sel, predOfSelect)
	...
	firstComNode := FlowNode{Node: firstCom.Comm}
	// fg.removeAllSuccessors(firstComNode)
	// fg.updateSuccForCommClause2(firstCom, firstComNode, actualSuccs[succIdx].Node)
	// succIdx++

	allCaseEntries := make([]IFlowNode, 0, 5)
	for i := firstComIdx; i < n; i++ {
		// SelectStmt.Body contains comm clauses only
		// So, it is safe to cast here.
		cc := comms[i].(*ast.CommClause)
		if cc.Comm != nil {
			...
			// Remove existing successors
			curr := FlowNode{Node: cc.Comm}
			allCaseEntries = append(allCaseEntries, curr)
			fg.removeAllSuccessors(curr)
			fg.updateSuccForCommClause2(cc, curr, actualSuccs[succIdx].Node)

			if succNode, ok := actualSuccs[succIdx].Node.(FlowNode); ok {
				if succNode.Node == firstCom.Comm {
					fg.addToPendingMap(firstCom.Comm, cc.Comm)
				}
			}

			if fg.hasEmptyBodyOrContinue(cc) && multiSuccessor {
				// Empty comm clause
				// There can be multiple successors
				// if this select statement is inside a loop
				// Check the first successor of the comm clause
				// If the first successor is the expression of the
				// first comm clause, this select is inside a loop
				// Keep the current comm clause pending on the
				// the first comm clause

				// Check the next successor
				succIdx++
				nextSucc := actualSuccs[succIdx]
				if flNode, ok := nextSucc.Node.(FlowNode); !ok || flNode.Node.Pos() > sel.Pos() {
					fg.addSuccessor(curr, nextSucc)
				}
			}
			succIdx++
		}
	...
	// We don't need to look for the default, as there will a
	// successor in the original successor list if it is present.
	if succIdx < len(actualSuccs) {
		old := actualSuccs[succIdx]
		allCaseEntries = append(allCaseEntries, old.Node)
	}

	successors := make([]SuccessorNode, 0, len(allCaseEntries))
	for _, caseEntry := range allCaseEntries {
		successors = append(successors, SuccessorNode{Node: caseEntry})
	}

	if len(predOfSelect) == 0 {
		fg.replaceSuccessor(nil, firstComNode, successors)
	} else {
		for _, pred := range predOfSelect {
			fg.replaceSuccessor(pred, firstComNode, successors)
		}
	}

	if pendings, ok := fg.pendingMap[firstCom.Comm]; ok {
		updatedNodes := set.OfElemets(predOfSelect...)
		pendings.ForEach(func(t ast.Node) {
			pred := FlowNode{Node: t}
			if updatedNodes.Contains(pred) {
				return
			}

			fg.replaceSuccessor(pred, firstComNode, successors)
		})
		delete(fg.pendingMap, firstCom.Comm)
		fg.pendingResolutionMap[firstCom.Comm] = successors
	}
}

func (fg *CFG) replaceSuccessor(pred IFlowNode, old IFlowNode, new []SuccessorNode) {
	if old == nil {
		return
	}

	existing := fg.successorMap[pred]
	var updated []SuccessorNode
	start := 0
	for idx, succ := range existing {
		if succ.Node == old {
			updated = append(updated, existing[start:idx]...)
	...
	preds := fg.predecessorMap[old]
	if len(preds) > 0 {
		updatedPreds := make([]IFlowNode, 0, len(preds))
		for _, oldPred := range preds {
			if oldPred != pred {
				updatedPreds = append(updatedPreds, oldPred)
			}
		}
		fg.predecessorMap[old] = updatedPreds
	}

	for _, sn := range new {
		fg.addSuccessor(pred, sn)
	}
	fg.successorMap[pred] = updated
}

func (fg *CFG) mayHaveMultipleSuccessor(selStmt *ast.SelectStmt, preds []IFlowNode) bool {
	parent := fg.ancestorMap[selStmt]
	for parent != nil {
		switch parentLoop := parent.(type) {
		case *ast.RangeStmt:
			body := parentLoop.Body
			return !fg.hasPredInsideRange(body.Pos(), selStmt.Pos(), preds)
		case *ast.BlockStmt:
			parent = fg.ancestorMap[parent]
		default:
			return false
		}
	}
...
func (fg *CFG) hasPredInsideRange(start, end token.Pos, preds []IFlowNode) bool {
	for _, pred := range preds {
		predPos := pred.(FlowNode).Node.Pos()
		if predPos > start && predPos < end {
			return true
		}
	}
	return false
...
func (fg *CFG) hasEmptyBodyOrContinue(cc *ast.CommClause) bool {
	if len(cc.Body) == 0 {
		return true
	}

	switch fstStmt := cc.Body[0].(type) {
	case *ast.BranchStmt:
		if fstStmt.Tok == token.CONTINUE {
			return true
		}
	}
	return false
...
func (fg *CFG) updateSuccForCommClause2(cc *ast.CommClause, pred, actualSucc IFlowNode) {
	if _, ok := cc.Comm.(*ast.AssignStmt); ok {
		...
		// m := <-c2 ----> m ----> res = m
		// We don't need the extra successor m.
		...
		succ := SuccessorNode{Node: actualSucc, SuccType: NormalPath}
